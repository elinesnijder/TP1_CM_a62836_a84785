...
class HistoryEntry(ft.Row):
    ...
    def __init__(self, index, expression, result, delete_callback,
        ...
        self.controls = [
            ...
            ft.Text(datetime.now().strftime("On %Y-%m-%d, at %H:%M:%S"),
                    size=10,
...
class CalculatorApp(ft.Container):
    ...
    def button_clicked(self, e):
        ...
        if self.result.value == "Error" or data == "AC":
            ...
            try:
                ...
                self.add_to_history(self.expression_text.value,
                                    self.result.value)
            except:
            ...
            try:
                ...
                self.result.value = self.format_number(str(round(
                    result, 2)))  #arredondamento
    ...
    def delete_history_entry(self, entry):
        ...
        self.update()
        print(
            f"Item do histórico mais antigo excluído com sucesso. -> {entry}")
    ...
    def save_history(self):
        history_data = [
            entry.controls[2].value for entry in self.history_list.controls
        ]
